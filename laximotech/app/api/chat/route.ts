...
ABOUT LAXIMOTECH.AI:
- Online learning platform offering 40+ courses at Rs 399 each
...
Answer helpfully and concisely in a friendly tone. Keep responses under 120 words. Always be encouraging. If you do not know something specific, suggest contacting info@laximotechsolutions.com or calling +91-8800236087.`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Groq full response:", JSON.stringify(rawData));

    const reply = rawData.choices?.[0]?.message?.content?.trim()
      || "Sorry, I could not process that. Please contact info@laximotechsolutions.com";
